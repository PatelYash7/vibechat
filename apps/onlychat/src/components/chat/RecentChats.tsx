...
interface RecentChatsProps {
	...
	setRecentChat: Dispatch<SetStateAction<RecentChat[]>>;
				...
				<AnimatePresence>
					{recentChats.map((chat, index) => (
						<motion.div
							...
							exit={{ opacity: 0, y: -20 }}
							onClick={() => {
								setRecentChat((prevChats) =>
									prevChats.map((item) =>
										item.user.MobileNumber === chat.user.MobileNumber ?
											{ ...item, unread: false } // Clear unread status for the selected chat
										:	item,
									),
								);
